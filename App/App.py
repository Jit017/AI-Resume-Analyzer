# AI Resume Analyzer — Prithwijit Choudhury (Streamlit)
...
# Configure via .env in project root or environment variables.
# Defaults match docker-compose.yml (MySQL on host port 3307).
_db_host = os.environ.get("MYSQL_HOST", "127.0.0.1")
_db_port = int(os.environ.get("MYSQL_PORT", "3307"))
_db_user = os.environ.get("MYSQL_USER", "root")
_db_password = os.environ.get("MYSQL_PASSWORD", "resume_analyzer_local")
_db_name = os.environ.get("MYSQL_DATABASE", "cv")
...
def run():
    ...
    if connection is None or cursor is None:
        ...
        st.error("**Cannot connect to MySQL.** The app needs a running database.")
        st.code(_db_connect_error or "unknown error", language="text")
        st.markdown(
            """
**If you use Docker (this repo):** start **Docker Desktop**, then in the project folder run:

```bash
docker compose up -d mysql
```

Wait until MySQL is healthy, then **refresh** this page.

**If you use your own MySQL:** set `MYSQL_HOST`, `MYSQL_PORT`, `MYSQL_USER`, `MYSQL_PASSWORD`, and `MYSQL_DATABASE` in `.env` (see `.env.example`).
"""
        )
        return
    ...
    link = '<b>Built with care by <span style="color: #021659;">Prithwijit Choudhury</span></b>' 
    ...
    ###### CODE FOR ABOUT PAGE ######
        ...
        <p align="justify">
            <b>Author:</b> Prithwijit Choudhury<br/><br/>
            Uses open-source libraries including <a href="https://github.com/OmkarPathak/pyresparser" style="text-decoration: none; color: grey;">pyresparser</a> for PDF parsing.
        </p>
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin@resume-analyzer':
                ...
                st.success("Welcome Prithwijit! Total %d " % values + " users have used this tool.")                
